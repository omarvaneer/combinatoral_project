...
class ExhaustiveSearch():
    def __init__(self,dataFile,k):
        ...
        self.k=np.min(k,self.data.size)#number of subsets threshold

        #combination generator initial condition  
        self.subset_group = np.zeros(self.k)
        self.coolex_b = np.zeros(self.m,dtype=bool)
        for i in range(k):
    ...
    def iterate(self):
        #check first
        if self.validateSets(self.subsetGroup):
            return 1 #found
        else:
            if self.coolex_x<self.m:
    ...
    def nextCombination(self):
        self.coolex_b[int(self.coolex_x-1)] = False
        self.coolex_b[int(self.coolex_y-1)] = True
        self.coolex_b[0] = self.coolex_b[int(self.coolex_x)]
        self.coolex_b[int(self.coolex_x)] = 1
        ...
        self.subset_group=np.nonzero(self.coolex_b)
