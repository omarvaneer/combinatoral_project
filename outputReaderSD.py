...
data = pd.DataFrame(columns=['name','time','result','min k','runtime','iterations','m (true set)','n (number of subsets)'])
output_folder = "steepestDescent_output"
output_folder2 = "steepestDescent_output2"
...
for filename in os.listdir(output_folder):
    ...
    file2 = open(os.path.join(output_folder2,filename), "r")
    iterations = int(file2.readline())   
    runtime = float(file2.readline())
    ...
    data.loc[counter] = [metadata[0],metadata[1][1:],res,minK,runtime,iterations,m,n]
    counter +=1
    file.close()
